...
module.exports = {
	...
	roomData: {
		...
		roomID: 0, // 匹配的房间号 访房间号为 对应配置文件的标识号 如 1 是新手场  2 是初级场
		gemPlayNum: 0, //U钻场第几局
	...
	_dealRoomMulData(data, isNotifySHowCardMulChange = true) {		
		...
		if (data.mul.u1 ) { 
			...
			if (this.roomData.leftPlayData.deskPos == 1) 	{	
				this.roomData.leftPlayData.doubleMul = data.mul.u1;
			}else if (this.roomData.rightPlayData.deskPos == 1){				
				this.roomData.rightPlayData.doubleMul = data.mul.u1;
			}else{				
				this.roomData.selfPlayData.doubleMul = data.mul.u1;
			}
		}
		...
		if (data.mul.u2) {
			...
			if (this.roomData.leftPlayData.deskPos == 2) 	{	
				this.roomData.leftPlayData.doubleMul = data.mul.u2;
			}else if (this.roomData.rightPlayData.deskPos == 2){				
				this.roomData.rightPlayData.doubleMul = data.mul.u2;
			}else{				
				this.roomData.selfPlayData.doubleMul = data.mul.u2;
			}
		}
		if (data.mul.u3) {
			
			this.roomData.mul.u3 = data.mul.u3;
			if (this.roomData.leftPlayData.deskPos == 3) 	{	
				this.roomData.leftPlayData.doubleMul = data.mul.u3;
			}else if (this.roomData.rightPlayData.deskPos == 3){				
				this.roomData.rightPlayData.doubleMul = data.mul.u3;
			}else{				
	...
	_dealEnterLandlord(landlord) {
		if (landlord == this.roomData.leftPlayData.deskPos) {
		...
		} else if (landlord == this.roomData.rightPlayData.deskPos) {
	...
	_dealSendPokerData(data) {
		...
		if (leftShowCardArray && leftShowCardArray.length > 0) {
			// 防止空字符串的问题""
	...
	judgeGameIsWin(winRoleType) {
		...
		if (selfIsLandLord) {
			if (winRoleType == Poker.GameRoleType.LandLord) {
				//胜利:  自己是地主,赢得是地主
				isWin = true;
			} else if (winRoleType == Poker.GameRoleType.Farmer) {
				//失败:  自己是地主,赢得是农民
				isWin = false;
			}
		} else {
	...
	initGameDataEvent() {
		...
		// 更新金币
		ObserverMgr.addEventListener(
			GameNetMsg.recv.UpdateMoney.msg,
			function(msg, data) {
				// 金币
				if (data.up) {
		...
		ObserverMgr.addEventListener(
			...
			function(msg, data) {
				...
				if(this.roomData.roomID != data.room){
				...
				if (leftPlayData) {
					this.roomData.leftPlayData.deskPos = leftPlayData.pos
					this.roomData.leftPlayData.name = leftPlayData.name; //decodeURI(leftPlayData.name);
				...
				if (rightPlayData) {
					this.roomData.rightPlayData.deskPos = rightPlayData.pos
					this.roomData.rightPlayData.name = rightPlayData.name; //decodeURI(rightPlayData.name);
		...
		// 初始化比赛场积分
		ObserverMgr.addEventListener(
			GameLocalMsg.Play.OnGetRaceInitScore,
			function(msg, data) {
		...
		// 叫地主
		ObserverMgr.addEventListener(
			GameNetMsg.recv.ShoutLandlord.msg,
			function(msg, data) {
		...
		// 抢地主
		ObserverMgr.addEventListener(
			GameNetMsg.recv.RobLandlord.msg,
			function(msg, data) {
		...
		// 加倍
		ObserverMgr.addEventListener(
			GameNetMsg.recv.ShoutDouble.msg,
			function(msg, data) {
		...
		// 玩家出牌
		ObserverMgr.addEventListener(
			GameNetMsg.recv.PlayerOutPoker.msg,
			function(msg, data) {
				var upUser = data.user;	

				var upUserCard = data.cards.toString();
				...
				if (nextUser != this.roomData.leftPlayData.deskPos && 
					nextUser != this.roomData.rightPlayData.deskPos) {
					// 轮到自己出牌了
					if (this.roomData.attackPoker.user != this.roomData.leftPlayData.deskPos &&
						this.roomData.attackPoker.user != this.roomData.rightPlayData.deskPos) {
						// 上次出的牌是自己的,那么自己就拿到了出牌权
						this.roomData.selfPlayData.isBeganPriority = true;
						//console.log("新的一轮开始了");
					} else {
						// 上次出的牌不是自己的,那么接着上家的牌出
						this.roomData.selfPlayData.isBeganPriority = false;
					}
				} else {
				}
		...
		// 托管
		ObserverMgr.addEventListener(
			GameNetMsg.recv.EntrustPlay.msg,
			function(msg, data) {
				var user = data.user;
		...
		// 明牌
		ObserverMgr.addEventListener(
			GameNetMsg.recv.DisplayPoker.msg,
			function(msg, data) {
				...
				if (user == this.roomData.leftPlayData.deskPos) {
					this.roomData.leftPlayData.isShowCard = true;
					this.roomData.leftPlayData.showCardArray = showCardArr;
				} else if (user == this.roomData.rightPlayData.deskPos) {
					this.roomData.rightPlayData.isShowCard = true;
		...
		//  金豆场 结算
		ObserverMgr.addEventListener(
			GameNetMsg.recv.GameOver_NormalMatch.msg,
			function(msg, data) {
				// 服务器结算校验
				...
				switch (this.roomData.leftPlayData.deskPos){
					case 1:
						this.roomData.leftPlayData.gold = data.u1;
						this.roomData.rightPlayData.gold = data.u3;
						this.playData.gold = data.u2;
						break;
					case 2:
						this.roomData.leftPlayData.gold = data.u2;
						this.roomData.rightPlayData.gold = data.u1;
						this.playData.gold = data.u3;
					break;
						case 3:
						this.roomData.leftPlayData.gold = data.u3;
						this.roomData.rightPlayData.gold = data.u2;
						this.playData.gold = data.u1;
						break;
				}
		...
		// U钻场单局 结算
		ObserverMgr.addEventListener(
			GameNetMsg.recv.GameOver_RoundMatch.msg,
			function(msg, data) {
				this.roomData.playState = Poker.GamePlayState.GameOver;
		...
		ObserverMgr.addEventListener(
			...
			function(msg, data) {
				...
				if (data.ticket){ //如有人跳跑   返还自己的门票
					this.playData.gold += data.ticket
				}
			},
			this
		);
		// 定时器时间到
		ObserverMgr.addEventListener(
			Poker.Event.TimerComplete,
			function(msg, data) {
				var isRobot = this.roomData.selfPlayData.isRobot;
				if (isRobot == false) {
					if (
						data == GameLocalMsg.Play.OnTimeOverPutCardWithAfter ||
		...
		ObserverMgr.addEventListener(
			...
			function(msg, data) {
				...
				if (currentDo == 1) {
					...
					// 本地构建一个出牌的消息
					if (putCardUser == currentUser) {
						// 如果要压制的牌是自己,又轮到自己出牌,说明是新的一局开始,
						putCard = [];
						putCardUser = 0;
					}
					var num = 0;
